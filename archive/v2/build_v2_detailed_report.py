...
def main() -> None:
    ...
    artifact["manifest"]["blocks"].insert(1, {"id": "v3_correction", "type": "markdown", "body": "## V3更正：V2补偿有效性结论撤回\n\nX接VREF的V2方案在WRITE后留下理想阈值残差−κ。旧功能门未检查跨阶段阈值系数；以下内容为历史输出，应以results_v3/REPORT.md为准。"})
    ...
    warning = "> **V3更正：本报告的阈值补偿有效性结论已撤回。** X连接VREF的V2方案在WRITE后留下理想阈值残差−κ；旧功能门未检查该项。参见[最新V3报告](../../../results_v3/REPORT.md)。以下保留历史结果。\n\n"
    (RESULTS / "REPORT.md").write_text(warning + "\n".join(md), encoding="utf-8")
